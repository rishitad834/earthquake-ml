...
def main():
    # Define parameters as variables (FIX FOR THE ERROR)
    n_wolves = 30
    max_iter = 50
    ...
    # Apply GWO feature selection (using the defined variables)
    selected_indices, selected_features, convergence_curve = gwo_feature_selection(
    ...
    print("\n" + "="*60)
    print("LIGHTGBM WITH GWO-SELECTED FEATURES")
    print("="*60)
    ...
    lgbm_gwo = lgbm.LGBMRegressor(random_state=42, verbose=-1)
    ...
    if hasattr(best_gwo_model, 'feature_importances_'):
        importances = best_gwo_model.feature_importances_
    ...
    plt.title(f'GWO Feature Selection\n({X_train_df.shape[1]} → {len(selected_features)} features)')
...
Search Space: {X_train_df.shape[1]}D
Hierarchy: Alpha, Beta, Delta, Omega
━━━━━━━━━━━━━━━━━━━━━
Results:
✓ Features: {X_train_df.shape[1]} → {len(selected_features)}
✓ Reduction: {((X_train_df.shape[1] - len(selected_features))/X_train_df.shape[1])*100:.1f}%
...
✓ Improvement: {((mse_baseline - mse_gwo)/mse_baseline)*100:.2f}%"""
    ...
    selected_features_df.to_csv('gwo_lightgbm_selected_features.csv', index=False)
    ...
    convergence_df.to_csv('gwo_lightgbm_convergence.csv', index=False)
    
    # Save models
    joblib.dump(best_baseline, 'lightgbm_baseline_model.pkl')
    joblib.dump(best_gwo_model, 'lightgbm_gwo_model.pkl')
    joblib.dump(scaler, 'feature_scaler_gwo.pkl')
    
    print(f"\nFiles saved:")
    print(f"  • gwo_lightgbm_selected_features.csv")
    print(f"  • gwo_lightgbm_convergence.csv")
    print(f"  • lightgbm_baseline_model.pkl")
    print(f"  • lightgbm_gwo_model.pkl")
    print(f"  • feature_scaler_gwo.pkl")
    ...
    print(f"Search Space Dimension: {X_train_df.shape[1]}")
    print(f"Final MSE: {mse_gwo:.4f}")
    print(f"Feature Reduction: {((X_train_df.shape[1] - len(selected_features))/X_train_df.shape[1])*100:.1f}%")
