...
class DecisionTreeOptimizationComparison:
    ...
    def run_original_decision_tree(self):
        """Run original Decision Tree with all features"""
        print("\n" + "="*60)
        print("RUNNING ORIGINAL DECISION TREE (BASELINE)")
        print("="*60)
        ...
        # Store results
        self.results['Original_DecisionTree'] = {
            'MSE': mse,
            'MAE': mae,
            'R2': r2,
        ...
        self.execution_times['Original_DecisionTree'] = execution_time
        self.feature_selections['Original_DecisionTree'] = list(range(self.X_train.shape[1]))
        
        print(f"Original Decision Tree Results:")
        print(f"  MSE: {mse:.4f}")
        print(f"  MAE: {mae:.4f}")
        print(f"  R²: {r2:.4f}")
    ...
    def run_all_comparisons(self):
        ...
        print("\n" + "#"*80)
        print("COMPREHENSIVE DECISION TREE FEATURE SELECTION COMPARISON")
        print("4 APPROACHES: Original Decision Tree, WOA, PSO, GWO")
        print("#"*80)
        
        # 1. Original Decision Tree
        self.run_original_decision_tree()
    ...
    def create_comparison_summary(self):
        ...
        baseline_mse = self.results['Original_DecisionTree']['MSE']
    ...
    def visualize_comparisons(self, df_comparison):
        ...
        plt.suptitle('Decision Tree Performance Metrics Comparison', fontsize=16, fontweight='bold')
        ...
        plt.suptitle('Decision Tree Feature Analysis & Algorithm Convergence', fontsize=16, fontweight='bold')
        plt.tight_layout(rect=[0, 0, 1, 0.96])
        plt.show()
        ...
        plt.suptitle('Decision Tree: Actual vs Predicted Earthquake Magnitude', fontsize=16, fontweight='bold')
        plt.tight_layout(rect=[0, 0, 1, 0.96])
        plt.show()
    ...
    def save_results(self):
        ...
        df_comparison = pd.DataFrame(comparison_data)
        df_comparison.to_csv('decision_tree_optimization_comparison_results.csv', index=False)
        
        # Save selected features for each method
        for method, features in self.feature_selections.items():
            if method != 'Original_DecisionTree':
                feature_names = self.X_train.columns[features].tolist()
                pd.DataFrame({
                    'Feature_Index': features,
                    'Feature_Name': feature_names
                }).to_csv(f'{method.lower()}_decision_tree_selected_features.csv', index=False)
        
        # Save convergence curves
        convergence_df = pd.DataFrame(dict([(k, pd.Series(v)) for k, v in self.convergence_curves.items()]))
        convergence_df.to_csv('decision_tree_convergence_curves.csv', index=False)
        
        # Save models
        for method, results in self.results.items():
            joblib.dump(results['Model'], f'{method.lower()}_decision_tree_model.pkl')
        
        print("\n" + "="*60)
        print("RESULTS SAVED:")
        print("  • decision_tree_optimization_comparison_results.csv")
        print("  • [algorithm]_decision_tree_selected_features.csv (for WOA, PSO, GWO)")
        print("  • decision_tree_convergence_curves.csv")
        print("  • [method]_decision_tree_model.pkl (all models)")
        print("="*60)
...
def main():
    ...
    # Create DataFrames with feature names preserved
    ...
    comparison = DecisionTreeOptimizationComparison(X_train_df, X_test_df, y_train, y_test)
    ...
    print("\n" + "#"*80)
    print("COMPREHENSIVE DECISION TREE COMPARISON COMPLETE!")
