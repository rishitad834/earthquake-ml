...
class WhaleOptimizationAlgorithm:
    ...
    def fitness_function(self, solution, X_train, X_test, y_train, y_test):
        ...
        try:
            ...
            lgb = lgbm.LGBMRegressor(n_estimators=100, random_state=42, verbose=-1)
            lgb.fit(X_train_selected, y_train)
            
            y_pred = lgb.predict(X_test_selected)
...
class LightGBMOptimizationComparison:
    ...
    def run_original_lightgbm(self):
        """Run original LightGBM with all features"""
        print("\n" + "="*60)
        print("RUNNING ORIGINAL LIGHTGBM (BASELINE)")
        print("="*60)
        ...
        lgb_model = lgbm.LGBMRegressor(random_state=42, verbose=-1)
        ...
        # Store results
        self.results['Original_LightGBM'] = {
            'MSE': mse,
            'MAE': mae,
            'R2': r2,
        ...
        self.execution_times['Original_LightGBM'] = execution_time
        self.feature_selections['Original_LightGBM'] = list(range(self.X_train.shape[1]))
        
        print(f"Original LightGBM Results:")
        print(f"  MSE: {mse:.4f}")
        print(f"  MAE: {mae:.4f}")
        print(f"  R²: {r2:.4f}")
    ...
    def run_all_comparisons(self):
        ...
        print("\n" + "#"*80)
        print("COMPREHENSIVE LIGHTGBM FEATURE SELECTION COMPARISON")
        print("4 APPROACHES: Original LightGBM, WOA, PSO, GWO")
        print("#"*80)
        
        # 1. Original LightGBM
        self.run_original_lightgbm()
    ...
    def create_comparison_summary(self):
        ...
        baseline_mse = self.results['Original_LightGBM']['MSE']
    ...
    def visualize_comparisons(self, df_comparison):
        ...
        plt.suptitle('LightGBM Performance Metrics Comparison', fontsize=16, fontweight='bold')
        ...
        plt.suptitle('LightGBM Feature Analysis & Algorithm Convergence', fontsize=16, fontweight='bold')
        plt.tight_layout(rect=[0, 0, 1, 0.96])
        plt.show()
        ...
        plt.suptitle('LightGBM: Actual vs Predicted Earthquake Magnitude', fontsize=16, fontweight='bold')
        plt.tight_layout(rect=[0, 0, 1, 0.96])
        plt.show()
    ...
    def save_results(self):
        ...
        df_comparison = pd.DataFrame(comparison_data)
        df_comparison.to_csv('lightgbm_optimization_comparison_results.csv', index=False)
        
        # Save selected features for each method
        for method, features in self.feature_selections.items():
            if method != 'Original_LightGBM':
                feature_names = self.X_train.columns[features].tolist()
                pd.DataFrame({
                    'Feature_Index': features,
                    'Feature_Name': feature_names
                }).to_csv(f'{method.lower()}_lightgbm_selected_features.csv', index=False)
        
        # Save convergence curves
        convergence_df = pd.DataFrame(dict([(k, pd.Series(v)) for k, v in self.convergence_curves.items()]))
        convergence_df.to_csv('lightgbm_convergence_curves.csv', index=False)
        
        # Save models
        for method, results in self.results.items():
            joblib.dump(results['Model'], f'{method.lower()}_lightgbm_model.pkl')
        
        print("\n" + "="*60)
        print("RESULTS SAVED:")
        print("  • lightgbm_optimization_comparison_results.csv")
        print("  • [algorithm]_lightgbm_selected_features.csv (for WOA, PSO, GWO)")
        print("  • lightgbm_convergence_curves.csv")
        print("  • [method]_lightgbm_model.pkl (all models)")
        print("="*60)
...
def main():
    ...
    comparison = LightGBMOptimizationComparison(X_train_df, X_test_df, y_train, y_test)
    ...
    print("\n" + "#"*80)
    print("COMPREHENSIVE LIGHTGBM COMPARISON COMPLETE!")
