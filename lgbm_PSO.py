...
def main():
    ...
    selected_indices, selected_features, convergence_curve = pso_feature_selection(
    ...
    print("\n" + "="*60)
    print("LIGHTGBM WITH PSO-SELECTED FEATURES")
    print("="*60)
    ...
    lgbm_pso = lgbm.LGBMRegressor(random_state=42, verbose=-1)
    ...
    if hasattr(best_pso_model, 'feature_importances_'):
        importances = best_pso_model.feature_importances_
        ...
        plt.title('Top 20 PSO-Selected Feature Importances (LightGBM)')
        show_plot()
    
    # Plot 5: PSO Swarm Behavior Visualization
    plt.figure(figsize=(12, 8))
    ...
    reduction = ((X_train_df.shape[1] - len(selected_features))/X_train_df.shape[1])*100
...
✓ Improvement: {((mse_baseline - mse_pso)/mse_baseline)*100:.2f}%"""
    ...
    selected_features_df.to_csv('pso_lightgbm_selected_features.csv', index=False)
    ...
    convergence_df.to_csv('pso_lightgbm_convergence.csv', index=False)
    
    # Save models
    joblib.dump(best_baseline, 'lightgbm_baseline_model.pkl')
    joblib.dump(best_pso_model, 'lightgbm_pso_model.pkl')
    joblib.dump(scaler, 'feature_scaler_pso.pkl')
    
    print(f"\nFiles saved:")
    print(f"  • pso_lightgbm_selected_features.csv")
    print(f"  • pso_lightgbm_convergence.csv")
    print(f"  • lightgbm_baseline_model.pkl")
    print(f"  • lightgbm_pso_model.pkl")
    print(f"  • feature_scaler_pso.pkl")
